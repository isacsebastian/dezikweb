<script lang="ts">
    ...
    onMount(() => {
        ...
        const draw = () => {
            ...
            const breathing = Math.sin(offset * 0.4) * 0.5 + 1;

            for (let i = 0; i < totalLayers; i++) {
                const step = i / totalLayers;

                // Colores rojos adaptados a tu marca
                const opacity = 0.03 + (step * 0.25);
                const hue = 355 + (i * 2); // Rojo con variación

                const gradient = ctx.createLinearGradient(0, 0, w, 0);
                gradient.addColorStop(0, `hsla(${hue}, 90%, 50%, 0)`);
                gradient.addColorStop(0.5, `hsla(${hue + 5}, 85%, 55%, ${opacity})`);
                ...
                const amplitude = (20 + (i * 12)) * breathing;
                const frequency = 0.001 + (i * 0.0003);
                const layerShift = i * 0.4;
                const mouseFactor = mouse.x * (i * 10);

                for (let x = 0; x <= w; x += 4) {
                    const sine = Math.sin(x * frequency + offset + layerShift);
                    const interference = Math.cos(x * 0.003 + offset * 0.5 + i);

                    const y = (h / 2) +
                              (sine * amplitude) +
                              (interference * (amplitude * 0.4)) +
                              (mouse.y * (i * 20));

                    if (x === 0) ctx.moveTo(x + mouseFactor, y);
                    else ctx.lineTo(x + mouseFactor, y);
                }
                ctx.stroke();
            }
